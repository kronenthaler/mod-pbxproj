...
class PBXCLIFile:
    @classmethod
    def add(cls, parser):
        file_parser = parser.add_parser(u'file', help=u'Add files to the project')
        ...
        file_parser.add_argument(u'--no-embed', u'-E', action=u'store_false', dest=u'embed',
                                 help=u'do not embed framework on the application')
        file_parser.add_argument(u'--sign-on-copy', u'-csoc', action=u'store_true', dest=u'code_sign_on_copy',
                                 help=u'code sign frameworks when copied to the application.')
        file_parser.add_argument(u'--ignore-unknown-types', u'-i', action=u'store_true', dest=u'ignore_unknown_types',
                                 help=u'ignore unknown types and add the file regardless.')
        file_parser.add_argument(u'--no-create-build-files', u'-C', action=u'store_false', dest=u'create_build_files',
                                 help=u'when adding a file, do not create any associated build file section required.')
        file_parser.set_defaults(func=command_parser(_add_file))

    @classmethod
    def remove(cls, parser):
        pass


def _add_file(project, args):
    ...
    for k in info:
        summary += u'{0} {1} sections created'.format(info[k], k)
    return summary
